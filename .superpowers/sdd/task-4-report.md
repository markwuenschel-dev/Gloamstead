...
- WorldForge typed failures (including production capability red) preserve their code, stage, retry disposition, and evidence reference through the Gloamstead operator boundary. Message-less failures use the typed code as fallback text rather than becoming generic runtime errors. Focused operator wiring, typed-red, and independent-verify path tests pass alongside the contract suite.
