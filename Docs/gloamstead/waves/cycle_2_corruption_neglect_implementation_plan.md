...
For an authored plan with a complete target contract (including Cycle II GardenRot), target resolution and confirmation must select and revalidate the exact PCG warning/subject/ritual/tag point **before** spawning an actor or mutating point state. A same-ritual nearer decoy is an ineligible target: it leaves no actor, no restored flag, no receipt, and player-facing invalid-target feedback. Generic point restoration remains available outside the interpretation route, but authored player confirmation may never consume a semantically mismatched place.
