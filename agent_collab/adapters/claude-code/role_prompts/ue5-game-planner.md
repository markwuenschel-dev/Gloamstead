...
description: Decomposes a UE5 game-development goal into a dependency DAG with exact file/content ownership, risk, required capabilities, docs impact, and binary/checkable acceptance criteria (minimal roster). Consults workflow_activation.json. Architecture/research via playbooks/. Read-only. Returns the plan; the Orchestrator turns it into handoffs.
model: inherit
